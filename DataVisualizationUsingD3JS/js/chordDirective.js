  ...
  var link = function ($scope, $el, $attr) {
    ...
    var matrix = matrixFactory.chordMatrix()
      ...
      .filter(function (item, r, c) {
        return (item.Giver === r.name && item.Receiver=== c.name) ||
               (item.GIver === c.name && item.Receiver === r.name);
      })
        ...
        if (!items[0]) {
          ...
          value = items.reduce(function (m, n) {
            if (r === c) {
              ...
              return m + (n.Giver === r.name ? n.Amount: n.Amount);
            }
          }, 0);
        }
    ...
    $scope.drawChords = function (data) {
      ...
      gEnter.append("path")
        ...
        .style("fill", function (d) {return String($scope.colors(d._id)); })
        .attr("d", arc);
      ...
      groups.exit().select("text").attr("fill", "teal");
      ...
      chords.enter().append("path")
        ...
        .style("fill", function (d) {
          console.log("Data is "+d._id);

          return $scope.colors(d._id); })
        .attr("d", path)
